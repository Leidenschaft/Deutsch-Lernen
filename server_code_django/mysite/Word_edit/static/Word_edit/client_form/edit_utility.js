...
$(document).ready(function () {
    ...
    $('form').on('submit', function (e) {
        e.preventDefault();
        $(this).ajaxSubmit({
            target: 'Word_edit/results/zhaofeng-shu33/',
...
/*    $("#submit_button").on('click', function () {
        if (Stichwort_input_boolean && Pluralform_input_boolean && GenitivSingular_input_boolean) {
            $("#errorMessage").html("");
            $.ajax({
                url: '../client_form/editing_interface.html',
                type: 'GET',
                dataType: 'html',
                timeout: 1000,
                cache: false,
                ...
                success: function (xml) {
                    $(self.parent.document).find("frame[name='navigation']").html(xml);
           ...
           // $("form").submit();rewritten with ajax
        }
        else {
            html_content = '';
